...
module.exports = function(shortname, body, callback) {
  $ = cheerio.load(body);
  ...
  courseBlocks.forEach(function (element) {
      ...
      course.sections = element.map(function(index, e) {
        ...
        var timeText = timeElement.html().trim().replace(/&nbsp;/g, '').trim();
        ...
        if (timeText == 'TBA' || !timeText) {
          ...
          var times = timeText.split('<br>');

          times.forEach(function (element, index, array) {
            element = element.trim();
            if (element == 'TBA' || element.length == 0) {
              array[index] = null;
            } else {
        ...
        var placeRegex = /(\w|\s)+(?=<|$)/g;
        var placeElement = e.eq(cfg.sectionOffsets.place);
        var placeText = placeElement.html();
        if (!(placeText == 'TBA' || !placeText || !times)) {
          placeText = placeText.replace('</a>', '');
          var places = placeText.match(placeRegex);
          times.forEach(function (element, index, array) {
            if (element) {
  ...
  console.log('Parsed ' + department.deptShortName);
