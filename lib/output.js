module.exports = {
    ...
    toDB: function (connection, async, results, next) {
        ...
        var saveQuarters = function (item, callback) {
            ...
            async.eachSeries(item.departments, saveDepartment(quarter), onComplete(callback));
        }
        ...
        var saveMeetingsAndFinal = function (meetings, coursefinal, callback) {
        ...
        var saveSection = function (courseId) {
            return function (item, callback) {
                ...
                connection.query('INSERT INTO sections SET ? ON DUPLICATE KEY UPDATE ? , section_id = LAST_INSERT_ID(section_id)',
                        [info, info],
                        saveMeetingsAndFinal(item.times, item.coursefinal, callback));
            }
        }

        var saveSections = function (sections, callback) {
            return function (err, result) {
                if (err) throw err;

                if (sections) {
                    async.eachSeries(sections, saveSection(result.insertId), onComplete(callback));
                } else {
                    callback(null);
                }
            }
        ...
        var saveCourse = function(deptId) {
            return function (item, callback) {
                var info = {
                    dept_id: deptId,
                ...
                connection.query('INSERT INTO courses SET ? ON DUPLICATE KEY UPDATE ? , course_id = LAST_INSERT_ID(course_id)',
                        [info, info],
                        saveSections(item.sections, callback));
            }
        }

        var saveCourses = function (courses, callback) {
            return function (err, result) {
                if (err) throw err;

                if (courses) {
                    async.eachSeries(courses, saveCourse(result.insertId), onComplete(callback));
                } else {
                    callback(null);
                }
            }
        ...
        var saveDepartment = function (quarter) {
            return function (item, callback) {
                ...
                connection.query('INSERT INTO departments SET ? ON DUPLICATE KEY UPDATE ? , dept_id = LAST_INSERT_ID(dept_id)',
                        [info, info],
                        saveCourses(item.courses, callback));
            }
        }
