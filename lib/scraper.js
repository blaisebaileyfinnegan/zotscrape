module.exports = function Scraper(cheerio, cfg, debug) {
	...
	this.department = function(shortname, body) {
        ...
        courseBlocks.forEach(function (element) {
            ...
            var regex = /\S*\d+\S*/;
            var courseNumber = regex.exec(line);
            ...
            course.sections = element.map(function(index, e) {
                ...
                var multipleRegex = /(p|\d)[\sMWFTuh](?!$)/;
                var daysRegex = /^[MWFTuTh]+(?=\s)/;
                ...
                if (timeText == 'TBA') {
                    ...
                    if (multipleRegex.test(timeText)) {
                        // We have a class with variable dates (ugh..)
                        var index;
                        while ((index = timeText.search(multipleRegex)) > -1) {
                            times.push(timeText.slice(0, index + 1));
                            timeText = timeText.slice(index + 1).trim();
                        }
                    }
                    times.push(timeText);
